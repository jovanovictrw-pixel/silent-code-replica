...
export function ProductCard({ product }: ProductCardProps) {
  return (
    <div className="sc-prod-card sc-fade-target group border border-white/5 bg-[#050505]">
      <div className="sc-prod-img-wrap overflow-hidden bg-[#0a0a0a]">
        ...
        <button className="sc-prod-add-btn backdrop-blur-md bg-white/90 hover:bg-white text-black font-bold py-4">
          ADD TO BAG
        </button>
        
        <div className="flex flex-col gap-2 absolute top-4 left-4 z-10">
          {product.labels && product.labels.map(label => (
            <div key={label} className="bg-white/90 backdrop-blur-sm text-black text-[8px] px-2 py-1 tracking-[0.3em] font-black uppercase shadow-2xl skew-x-[-12deg]">
